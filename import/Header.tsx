...
const formatDateDisplay = (date: Date, viewMode: ViewMode): string => {
  ...
  const optionsFullDateWithYear: Intl.DateTimeFormatOptions = { month: 'short', day: 'numeric', year: 'numeric' };


  switch (viewMode) {
    case 'day':
      if (isSameDate(normalizedDate, today)) return `${normalizedDate.toLocaleDateString('en-US', optionsShort)}, Today`;
      const yesterday = new Date(today);
      yesterday.setDate(today.getDate() - 1);
      if (isSameDate(normalizedDate, yesterday)) return `${normalizedDate.toLocaleDateString('en-US', optionsShort)}, Yesterday`;
    ...
    case 'week':
      ...
      const startStr = start.toLocaleDateString('en-US', start.getFullYear() === today.getFullYear() ? optionsShort : optionsFullDateWithYear);
      const endStr = end.toLocaleDateString('en-US', end.getFullYear() === today.getFullYear() ? optionsShort : optionsFullDateWithYear);
      
      if (start.getFullYear() !== end.getFullYear()) {
         return `${start.toLocaleDateString('en-US', optionsFullDateWithYear)} - ${end.toLocaleDateString('en-US', optionsFullDateWithYear)}`;
      }
...
const Header: React.FC<HeaderProps> = ({ currentDate, viewMode, onViewModeChange, onPrevPeriod, onNextPeriod }) => {
  return (
    <header className="flex flex-col items-center text-slate-300 space-y-4 sm:space-y-6">
      ...
      <div className="w-full relative flex items-center justify-center h-10">
        ...
        <ViewModeSwitcher currentViewMode={viewMode} onViewModeChange={onViewModeChange} />
      </div>
