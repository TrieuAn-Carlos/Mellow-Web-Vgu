...
import type { TimeDistributionItemData } from '../types';

interface TimeDistributionItemProps {
  item: TimeDistributionItemData;
}

const TimeDistributionItem: React.FC<TimeDistributionItemProps> = ({ item }) => {
  return (
    <div className="flex items-center space-x-3 sm:space-x-4 my-3.5">
      ...
      <div className="w-20 sm:w-28 text-right flex-shrink-0">
        <div className="text-xs sm:text-sm font-semibold text-white truncate" title={item.category}>
          {item.category}
        </div>
        <div className="text-xxs sm:text-xs text-slate-400">{item.duration}</div>
      </div>
    </div>
  );
};
...
const TimeDistributionCard: React.FC<TimeDistributionCardProps> = ({ data, className, style }) => {
  return (
    <div 
      className={`bg-slate-800 p-6 rounded-xl shadow-lg h-full ${className || ''}`}
      style={style}
    >
